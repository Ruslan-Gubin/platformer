...
export class Game {
  ...
  constructor(private pixiApp: Application, public assets: AssetsFactory) {
    this.worldContainer = new World();
    this.pixiApp.stage.addChild(new StaticBackground(this.pixiApp.screen, this.assets));
  ...
  /** проверка на велет за пределы экрана и врага и пули */
  private checkEntityStatus(entity: EntityArrayItemType, index: number) {
    ...
    if (isRemove) this.removeEntity(entity, index);
  }
